...
try:
    ...
    from PySide6.QtWidgets import (
        ...
        QMessageBox, QFrame, QTextEdit, QLineEdit, QCheckBox # Added QCheckBox
    )
...
def get_potential_files_recursively(current_directory, original_root_dir, script_name, include_logs=False):
    ...
    try:
        for item_entry in os.scandir(current_directory_path):
            ...
            if item.is_file():
                excluded_suffixes = [
                    # '.log', # *** REMOVED .log FROM HARDCODED EXCLUSIONS ***
            ...
            # --- Directory processing ---
                ...
                potential_files.extend(get_potential_files_recursively(item_abs_path, original_root_dir, script_name, include_logs))
...
class ScriptAggregatorApp(QWidget):
    def __init__(self, script_dir, script_name):
        ...
        self.include_logs_checkbox = QCheckBox("Include selected .log files in Step 1b output") # <-- NEW Checkbox
        ...
        self.initUI()
        self.populate_file_tree() # Now includes logs by default in the scan
    ...
    def initUI(self):
        ...
        # Enable drag-and-drop selection later if needed
        # self.tree_view.setDragDropMode(QtWidgets.QAbstractItemView.InternalMove)
        ...
        # Step 1b Mode
        mode_groupbox = QGroupBox("Step 1b: Select Final Action Mode & Options") # Renamed GroupBox
        ...
        self.include_logs_checkbox.setChecked(False) # Default to not including logs
    ...
    def populate_file_tree(self):
        ...
        for rel_path in potential_files:
            ...
            if rel_path.suffix.lower() == '.log':
                file_item.setIcon(self.log_icon) # Use specific icon if desired
            else:
    ...
    def generate_planning_request(self):
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                # --- Part 3: Project File Tree (excluding logs) ---
                ...
                # --- Part 4: Concatenated Project Source Code (excluding logs) ---
                f.write("<Project Source Code (excluding .log files)>\n\n")
                for rel_path in sorted_all_files: # Use the no_logs list
    ...
    def apply_suggested_files(self):
        ...
        # Robust parsing of comma-separated list, handling spaces and quotes
        try:
            # Use regex to handle potential quotes and extra spaces
            suggested_paths_raw = re.findall(r'[^,\s"]+|"[^"]*"', suggestions_text)
            suggested_paths_raw = [p.strip().strip('"') for p in suggested_paths_raw if p.strip()] # Clean up
        except Exception as e:
        ...
        for p_raw in suggested_paths_raw:
            ...
            if p_norm: # Ensure not empty after stripping
        ...
        for norm_path in suggested_paths_normalized:
            ...
            if item and item.isCheckable():
                item.setCheckState(Qt.CheckState.Checked)
                found_count += 1
                # Optionally expand parent nodes to make the selected item visible
                parent = item.parent()
                while parent:
        ...
        if not_found:
            status_msg += f" ({len(not_found)} suggested files not found or not selectable: {', '.join(not_found[:3])}{'...' if len(not_found) > 3 else ''})"
    ...
    def generate_final_output_file(self):
        ...
        for item in self.item_path_map.values():
            if item.isCheckable() and item.checkState() == Qt.CheckState.Checked:
                ...
                if rel_path:
                     # Apply log filtering based on checkbox *before* adding to the list
                    if not include_logs_in_output and rel_path.suffix.lower() == '.log':
                        continue # Skip this log file if checkbox is unchecked
        ...
        # Add log status to filename for clarity
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                # Sort the final list of paths to be included
                ...
                for rel_path in sorted_rel_paths: # Use the potentially filtered list
                    abs_path = self.script_dir / rel_path
                    rel_path_str = rel_path.as_posix()
                    f.write(f"<file path='{rel_path_str}'>\n")
                    content = read_file_content(abs_path)
