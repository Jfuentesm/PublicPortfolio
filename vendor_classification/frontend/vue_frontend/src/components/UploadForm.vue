<!-- <file path='frontend/vue_frontend/src/components/UploadForm.vue'> -->
  ...
  const handleUpload = async () => {
    ...
    try {
      // --- UPDATED: Use apiService.uploadFile ---
      const response = await apiService.uploadFile(formData);
      // --- END UPDATED ---
      // Use the JobResponse structure returned by the API
      successMessage.value = `Upload successful! Job ${response.id} started (Target Level: ${response.target_level}). Monitoring status below...`;
