...
class ScriptAggregatorApp(QWidget):
    ...
    def generate_final_output_file(self):
        ...
        for item in self.item_path_map.values():
            if item.isCheckable() and item.checkState() == Qt.CheckState.Checked:
                ...
                if rel_path: # Should always be true for items in map
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for rel_path in sorted_rel_paths:
                    f.write(f"- {rel_path.as_posix()}\n")
                if not include_logs_in_output and any(p.suffix.lower() == '.log' for p in self.item_path_map.keys()):
                     # Add note only if logs exist but were excluded
    ...
    def undo_last_apply(self):
        ...
        if not self.temp_backup_dir:
             QMessageBox.warning(self, "Undo Error", "Cannot undo: Backup directory information is missing.")
