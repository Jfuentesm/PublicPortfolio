...
class ScriptAggregatorApp(QWidget):
    def __init__(self, script_dir, script_name):
        ...
        self.log_icon = self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_FileIcon)
    ...
    def initUI(self):
        self.setWindowTitle("Script Aggregator & Applier (with Undo)") # Updated title
        self.setGeometry(150, 150, 1050, 900) # Increased height slightly for undo button
        ...
        # --- Add Apply and Undo buttons side-by-side ---
        ...
        self.undo_button = QPushButton("Undo Last Apply") # <-- NEW Undo Button
        self.undo_button.clicked.connect(self.undo_last_apply) # <-- Connect Slot
        self.undo_button.setEnabled(False) # <-- Initially disabled
        apply_undo_layout.addWidget(self.apply_changes_button, 2) # Give Apply more space
        apply_undo_layout.addWidget(self.undo_button, 1)
        step2_layout.addLayout(apply_undo_layout)
        # --- End Apply/Undo layout ---
        ...
        main_h_layout.addWidget(right_widget, 4)
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for rel_path in sorted_all_files:
                    ...
                    if content is not None: f.write(content) # Check if read was successful
                    f.write("\n</file>\n\n")
    ...
    def apply_suggested_files(self):
        ...
        for p_raw in suggested_paths_raw:
            ...
            if p_norm: suggested_paths_normalized.add(p_norm)

        if not suggested_paths_normalized:
             QMessageBox.warning(self, "Parsing Error", "Could not parse valid file paths.")
             return
        ...
        if not_found:
            ...
            QMessageBox.warning(self, "Partial Match", f"Applied suggestions, but some files were not found/selectable:\n\n- {chr(10).join(not_found)}\n\nVerify selection.")
    ...
    def generate_final_output_file(self):
        ...
        if not selected_relative_paths:
            msg = "Please select at least one file before executing Step 1b."
            if not include_logs_in_output: msg += "\nNote: .log files are currently excluded."
        ...
        try:
            output_dir.mkdir(parents=True, exist_ok=True)
            with open(output_file, 'w', encoding='utf-8') as f:
                # Goal, Instruction, File Tree, Code... (content as before)
                f.write('<goal or issue to address>\n' + goal_text + '\n</goal or issue to address>\n\n\n')
                f.write('<output instruction>\n' + selected_mode["output_instruction"] + '\n</output instruction>\n\n\n')
                f.write("<Tree of Included Files>\n")
                sorted_rel_paths = sorted(selected_relative_paths, key=lambda p: p.as_posix())
                for rel_path in sorted_rel_paths: f.write(f"- {rel_path.as_posix()}\n")
                if not include_logs_in_output: f.write("\n(Note: .log files were excluded...)\n")
                f.write("</Tree of Included Files>\n\n\n")
    ...
    def apply_llm_changes(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Parsing Error", f"Could not parse response.\nError: {e}")
            return

        if not file_blocks:
            self.status_label.setText("No file blocks found.")
            QMessageBox.information(self, "No Changes Found", "No `<file path='...'>...</file>` blocks found.")
            return

        self.status_label.setText(f"Found {len(file_blocks)} blocks. Validating paths...")
        QApplication.processEvents()
        ...
        for path_str, content in file_blocks:
            ...
            if not relative_path_str:
                 skipped_files.append("(Empty path provided)")
                 continue

            try:
                abs_path = (project_root_resolved / relative_path_str).resolve()
                # Security/Validation Checks (as before)
                if not abs_path.is_relative_to(project_root_resolved):
                    skipped_files.append(f"{relative_path_str} (Outside project)")
                    continue
                parent_dir = abs_path.parent
                if not parent_dir.is_dir(): # Check if parent exists and is a dir
                    skipped_files.append(f"{relative_path_str} (Parent dir invalid/missing)")
                    continue
                if abs_path.is_dir():
                    skipped_files.append(f"{relative_path_str} (Path is directory)")
                    continue

                # Add to list for confirmation dialog
                valid_changes_to_confirm.append({"abs_path": abs_path, "content": content, "rel_path_str": relative_path_str})

            except Exception as e:
                skipped_files.append(f"{relative_path_str} (Error: {e})")
                continue

        if not valid_changes_to_confirm:
            self.status_label.setText("No valid file paths found.")
            msg = "No valid file paths found in response."
            if skipped_files: msg += "\n\nSkipped:\n- " + "\n- ".join(skipped_files)
            QMessageBox.warning(self, "No Valid Changes", msg)
            return

        # --- Confirmation Dialog ---
        confirmation_message = f"Found {len(valid_changes_to_confirm)} valid file(s) to apply changes to:\n\n"
        ...
        confirmation_message += "\nBackups will be created for overwritten files. Proceed?" # Updated message
        ...
        for change in valid_changes_to_confirm:
            ...
            try:
                ...
                if abs_path.exists():
                    ...
                    backup_filename = f"{rel_path_str.replace('/', '_')}_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.bak"
                    backup_path = self.temp_backup_dir / backup_filename
                    shutil.copy2(abs_path, backup_path) # copy2 preserves metadata
                    print(f"Backed up '{rel_path_str}' to '{backup_path}'")
                    was_created = False
                else:
                ...
                # --- Write File ---
                abs_path.parent.mkdir(parents=True, exist_ok=True) # Ensure parent exists
                ...
                # Attempt to rollback this specific file if backup exists? (Could get complex)
            except Exception as e:
                failed_files.append(f"{rel_path_str} (Unexpected Error: {e})")
                print(f"Unexpected error processing file {rel_path_str}: {e}")

        # --- Final Report ---
        final_message = f"Applied changes: {success_count} file(s) updated/created."
        if failed_files:
            final_message += f"\nFailed: {len(failed_files)} file(s):\n- " + "\n- ".join(failed_files)
            QMessageBox.warning(self, "Partial Success", final_message)
        elif success_count > 0: # Only show success if something actually happened
             QMessageBox.information(self, "Success", final_message)

        if skipped_files: final_message += f"\n({len(skipped_files)} skipped paths)"

        self.status_label.setText(final_message)

        # --- Enable Undo Button if successful changes were made ---
        if success_count > 0:
            self.undo_button.setEnabled(True)
        else:
            # If all failed, cleanup the (likely empty) backup dir
    ...
    def undo_last_apply(self):
        ...
        if not self.last_applied_changes or self.temp_backup_dir is None:
            QMessageBox.information(self, "Undo", "No changes to undo.")
            return

        reply = QMessageBox.question(self, "Confirm Undo",
                                     f"Undo the last {len(self.last_applied_changes)} file change(s)?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)
        ...
        for change_info in self.last_applied_changes:
            ...
            rel_path_str = original_path.relative_to(self.script_dir).as_posix() # For messages
            ...
            try:
                if was_created:
                    ...
                    if original_path.exists():
                        ...
                        print(f"Undo: Newly created file '{rel_path_str}' already deleted.")
                        undo_success_count += 1
                else:
                    # If the file was overwritten, restore from backup
                    if backup_path and backup_path.exists():
                        shutil.copy2(backup_path, original_path) # Restore
                        print(f"Undo: Restored '{rel_path_str}' from backup.")
                        undo_success_count += 1
                    elif backup_path:
                        # Backup missing! Cannot restore.
                        undo_failed_files.append(f"{rel_path_str} (Backup missing: {backup_path})")
                        ...
                        # Should not happen if was_created is False, but handle defensively
                         undo_failed_files.append(f"{rel_path_str} (Invalid state: Overwritten but no backup path)")
                         print(f"Error: Cannot undo '{rel_path_str}', invalid state (no backup path).")

            except (IOError, OSError, shutil.Error) as e:
                 undo_failed_files.append(f"{rel_path_str} (Error: {e})")
                 print(f"Error undoing change for {rel_path_str}: {e}")
            except Exception as e:
                 undo_failed_files.append(f"{rel_path_str} (Unexpected Error: {e})")
        ...
        if undo_failed_files:
            final_message += f"\nFailed to undo {len(undo_failed_files)} file(s):\n- " + "\n- ".join(undo_failed_files)
        ...
        # Optional: Refresh tree view after undo
        # self.populate_file_tree()
