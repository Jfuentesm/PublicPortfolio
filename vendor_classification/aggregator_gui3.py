...
def get_potential_files_recursively(current_directory, original_root_dir, script_name):
    ...
    try:
        for item_entry in os.scandir(current_directory_path):
            ...
            # Exclude common build/env dirs, archive dirs, and our own output files
            if item.name in ['.venv', 'venv', 'env', '__pycache__', 'node_modules', 'dist', 'build'] or \
               ...
               item.name.startswith('concat_'): # Exclude step 1b output files too
                continue
...
class ScriptAggregatorApp(QWidget):
    def __init__(self, script_dir, script_name):
        ...
        self.initUI()
        self.populate_file_tree() # Populates tree and self.item_path_map
    ...
    def populate_file_tree(self):
        self.model.clear()
        self.item_path_map.clear() # Clear the map too
        invisible_root = self.model.invisibleRootItem()
        folder_items = {'': invisible_root}

        # Get *all* potential files initially for the map and tree display
        ...
        for rel_path in potential_files:
            ...
            # Files are checkable for Step 1b, default to unchecked now maybe? Or checked? Let's keep checked.
    ...
    def generate_planning_request(self):
        ...
        QApplication.processEvents() # Show status update

        # --- Get ALL potential files for concatenation ---
        # We re-run this to ensure we get the latest list, though could use self.item_path_map keys too
        ...
        output_file_name = f"planning_request_{timestamp_str}.txt"
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                # Sort all potential files for the tree view
                ...
                for rel_path in sorted_all_files: # Use the same sorted list
            ...
            self.status_label.setText(f"Planning request file generated: {output_file_name}")
    ...
    def generate_final_output_file(self):
        ...
        for item in self.item_path_map.values(): # Iterate through stored file items
        ...
        timestamp_str = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file_name = f"concat_{self.script_dir.name}_{selected_mode_key}_{timestamp_str}.txt"
        output_file = self.script_dir / output_file_name

        self.status_label.setText(f"Generating final action file: {output_file_name}...")
        QApplication.processEvents()
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                f.write(selected_mode["issue_placeholder"]) # Use placeholder from selected mode
                f.write('</issue to address>\n\n\n')
            ...
            self.status_label.setText(f"Successfully generated final action file: {output_file_name}")
