...
class FileTreeFilterProxyModel(QSortFilterProxyModel):
    def __init__(self, item_path_map, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.filter_text = ''
        self.item_path_map = item_path_map
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        ...
        if not item:
            return False
        # Always show folders if any child matches
        if item.hasChildren():
            for i in range(item.rowCount()):
                if self.filterAcceptsRow(i, index):
                    return True
        # For files, match on name or full path
        if not self.filter_text:
            return True
        # Find the rel_path_str for this item
        for rel_path_str, mapped_item in self.item_path_map.items():
            if mapped_item is item:
                if self.filter_text in rel_path_str.lower() or self.filter_text in item.text().lower():
                    return True
        return False
...
class ScriptAggregatorApp(QWidget):
    def __init__(self, script_dir, script_name):
        ...
        self.proxy_model = FileTreeFilterProxyModel(self.item_path_map)
        ...
        # --- File Tree Filtering and Selection Management ---
        self.filter_input = QLineEdit()
        self.filter_input.setPlaceholderText("Filter files by name or path...")
        self.filter_input.textChanged.connect(self.filter_file_tree)
        self._current_filter = ''

        self.selections_dir = self.script_dir / ".selections"
        self.selections_dir.mkdir(exist_ok=True)
        self.selection_combo = QtWidgets.QComboBox()
        self.selection_combo.setMinimumWidth(120)
        self.load_selection_button = QPushButton("Load")
        self.save_selection_button = QPushButton("Save Selection As...")
    ...
    def initUI(self):
        ...
        # Add filter bar above the tree
        left_v_layout.addWidget(self.filter_input)
        ...
        # Add save/load selection controls below the tree
        selection_hbox = QHBoxLayout()
        selection_hbox.addWidget(self.selection_combo)
        selection_hbox.addWidget(self.load_selection_button)
        selection_hbox.addWidget(self.save_selection_button)
        left_v_layout.addLayout(selection_hbox)
    ...
    def filter_file_tree(self):
        """Filter the file tree based on the filter input."""
        filter_text = self.filter_input.text().strip()
        self._current_filter = filter_text
        self.proxy_model.setFilterText(filter_text)

    def refresh_selection_combo(self):
        self.selection_combo.clear()
        if self.selections_dir.exists():
            for f in sorted(self.selections_dir.glob('*.json')):
                self.selection_combo.addItem(f.stem, str(f))

    def save_selection(self):
        checked = [rel_path.as_posix() for rel_path, item in self.item_path_map.items() if item.isCheckable() and item.checkState() == Qt.CheckState.Checked]
        if not checked:
            QMessageBox.warning(self, "No Files Selected", "Please select at least one file to save a selection.")
            return
        # Suggest a name based on goal or timestamp
        import re, time
        goal = self.goal_input.toPlainText().strip()
        default_name = re.sub(r'[^a-zA-Z0-9_\-]+', '_', goal)[:32] if goal else f'selection_{int(time.time())}'
        name, ok = QtWidgets.QInputDialog.getText(self, "Save Selection As", "Enter a name for this selection:", text=default_name)
        if ok and name.strip():
            name = re.sub(r'[^a-zA-Z0-9_\-]+', '_', name.strip())
            path = self.selections_dir / f"{name}.json"
            import json
            with open(path, 'w', encoding='utf-8') as f:
                json.dump(checked, f, indent=2)
            self.refresh_selection_combo()
            QMessageBox.information(self, "Selection Saved", f"Selection saved as '{name}'.")

    def load_selection(self):
        idx = self.selection_combo.currentIndex()
        if idx < 0:
            QMessageBox.warning(self, "No Selection", "No saved selection to load.")
            return
        path = self.selection_combo.currentData()
        import json
        try:
            with open(path, 'r', encoding='utf-8') as f:
                checked = set(json.load(f))
        except Exception as e:
            QMessageBox.critical(self, "Load Error", f"Could not load selection: {e}")
            return
        # Deselect all, then check those in the loaded set
        self.select_none()
        found = 0
        for rel_path_str, item in self.item_path_map.items():
            if rel_path_str in checked and item.isCheckable():
                item.setCheckState(Qt.CheckState.Checked)
                found += 1
        self.status_label.setText(f"Loaded selection: {found} files checked.")
