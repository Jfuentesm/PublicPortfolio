...
<file path='src/frontend/package.json'>
{
    ...
    "dependencies": {
      ...
      "@tiptap/vue-3": "^2.0.0-alpha.67",
      "@tiptap/pm": "^2.0.0-alpha.4",          
      "@tiptap/starter-kit": "^2.0.0-alpha.88",
      "@tiptap/extension-markdown": "^2.0.0-alpha.14"
    },
    "devDependencies": {
      "@vitejs/plugin-vue": "^4.0.0",
      "vite": "^4.0.0",
...
<template>
  <div class="tiptap-container">
    ...
    <!-- We use EditorContent to render the Tiptap editor -->
...
//    - currentNoteContent is the Markdown text from your parent component
//    - updateContent is the event we fire whenever Tiptap content changes
...
// 2) Set up the editor using useEditor
//    - We pass your initial markdown content
//    - We add StarterKit + Markdown extension
//    - onUpdate triggers each time the user makes an edit
const editor = useEditor({
  extensions: [
    StarterKit,
    Markdown
  ],
  content: props.currentNoteContent,
  onUpdate({ editor }) {
    // Get the updated Markdown from the Tiptap extension
    const updatedMarkdown = editor.storage.markdown.getMarkdown()
    // Emit it up to the parent so it can be saved
    emit('updateContent', updatedMarkdown)
  }
})

// 3) Watch for changes from the parent prop "currentNoteContent"
//    If the parent changes the note externally, we reset the editor
watch(
  () => props.currentNoteContent,
  newVal => {
    if (!editor.value) return

    // Compare the parent's new markdown to what's in Tiptap
    const currentMarkdown = editor.value.storage.markdown.getMarkdown()
    if (newVal !== currentMarkdown) {
      // Overwrite Tiptap if they're different
      editor.value.commands.setContent(newVal)
    }
  }
)

// 4) Clean up the editor instance when unmounted
onBeforeUnmount(() => {
  if (editor.value) {
