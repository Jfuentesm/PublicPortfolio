<issue to solve>
The tables in the schemas are correctlyt defined, but the views in the app are not consistent with the tables. 
...
class LoggingMiddleware:
    ...
    def __call__(self, request):
        ...
        params = {k: v for k, v in request.GET.items() if k.lower() not in ('password', 'token', 'auth', 'key')}
        if params:
