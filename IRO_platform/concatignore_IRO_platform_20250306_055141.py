<issue to solve>
the 'High Priority IROs' table in the home screen loads correctly. howver, the IROs library table in IROs screen does not load content after I select the context. message:

Error loading IRO data.
Please try refreshing the page or selecting a different context.
Technical details: p.split is not a function. (In 'p.split(" ")', 'p.split' is undefined)
    ...
    try:
        ...
        for iro in iro_queryset:
            try:
                ...
                # Get other fields safely with explicit string conversion
                iro_type = str(getattr(iro, 'type', 'unknown'))
                esrs_standard = str(getattr(iro, 'esrs_standard', '') or '')
                current_stage = str(getattr(iro, 'current_stage', 'unknown'))
